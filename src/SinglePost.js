...
import MySketch from './4 copy.jpg';
    ...
    render(){
        ...
        if (this.state.loadedPost) {
            post = (
              ...
              <header>
                  <h1>{this.state.loadedPost.title}</h1>
                  <p> <span style={{display:'flex'}}>Posted on 
                  <span style={{paddingLeft:'5px',fontWeight:'bold'}}>
                  {this.state.Date}</span>
                      <span style={{color:'grey',paddingLeft:'50px'}}>By</span>
                      - <b>{this.state.author}</b></span></p>
                  <img src={MySketch} width="100%" height="auto" alt="post1" />
              </header>
              <div class="post-body">
                    <h2><span style={{fontWeight: '400'}}>{this.state.loadedPost.title}</span></h2>
                    <p><span style={{fontWeight: '400',fontSize:'20px'}}>{this.state.loadedPost.body}</span></p>
